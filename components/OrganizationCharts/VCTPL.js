...
const VCTPL = ({ attemptData }) => {
  ...
  const hoistTravelGraphangle =
    attemptData && attemptData.graphs
      ? attemptData.graphs.find(
          graph =>
            graph.name === "Swing angle  to highlighted zone while loading"
        )
      : null;
  ...
  const hoistTravelGraphangleunloading =
    attemptData && attemptData.graphs
      ? attemptData.graphs.find(
          graph =>
            graph.name === "Swing angle  to highlighted zone while stacking"
        )
      : null;
...
//   };
  ...
  const option = {
    ...
    title: [
      {
        left: "center",
        text: "Loading Performance Analysis",
  ...
  const option1 = {
    ...
    title: [
      {
        left: "center",
        text: "Stacking Performance Analysis",
        subtext: "Swing and Speed Metrics Over Time",
        textStyle: {
          color: "#34568B", // A sophisticated deep blue
          fontFamily: 'Segoe UI, Roboto, "Helvetica Neue", Arial, sans-serif',
