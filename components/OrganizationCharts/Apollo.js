...
const Apollo = ({ attemptData, attemptData2, compare = false }) => {
  ...
  if (attemptData && attemptData.kpis) {
    attemptData.kpis.forEach(kpi => {
      ...
      if (typeof kpi.ideal_time === "string") {
        ...
        const idealTimeValue = idealTimeMatch
          ? parseFloat(idealTimeMatch[0])
          : 0;
  ...
  kpiNames.forEach((name, index) => {
    combinedPieChartData.push({
      name: `Actual Time`,
      value: actualValues[index]
    });
    combinedPieChartData.push({
      name: `Ideal Time`,
      value: idealValues[index]
    });
  });
  ...
  const determineInsightText = kpis => {
    ...
    if (kpis && Array.isArray(kpis)) {
      ...
      return actualTimeExceeds
        ? { text: "task exceeded ideal time", color: "#ff3333" }
        : { text: "task completed within ideal time", color: "#33cc33" };
    };
    
  const position = determineInsightText(attemptData.kpis).text.includes(
    "exceeded"
  )
    ? "71.5%"
    : "69%"; // Adjust these percentages as needed

  const option1 = {
    title: {
        text: compare ? "Ideal VS Actual Time Comparison - User1" : "Ideal VS Actual Time Comparison",
        subtext: "Apollo"
      },
    graphic: {
      elements: [
          ...
          style: {
            ...
            backgroundColor: {
              ...
              colorStops: [
                {
                  offset: 0,
                  color: "rgba(247, 247, 247, 0.95)" // Light start of gradient
                },
                {
                  offset: 1,
                  color: "rgba(230, 230, 230, 0.95)" // Slightly darker end of gradient
                }
              ],
              global: false
            },
  ...
  const option2 = {
     title: {
    text: "Ideal VS Actual Time Comparison - User2",
    subtext: "Apollo"
  },
    graphic: {
      elements: [
        ...
        },
        {
          ...
          top: "50%",
          style: {
            text: determineInsightText(attemptData2?.kpis).text,
            fill: determineInsightText(attemptData2?.kpis).color,
            font: "bold 14px Arial",
            textAlign: "center",
            backgroundColor: "#454545", // White background
            borderColor: determineInsightText(attemptData2?.kpis).color,
            borderWidth: 2,
            borderRadius: 4,
            shadowBlur: 10,
            shadowColor: "rgba(0, 0, 0, 0.3)",
